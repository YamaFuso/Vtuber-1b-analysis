...
def main():
    ...
    if True:
        ...
        chat_path_list = chat_path_list[index:]
        print(chat_path_list)
        ...
        for each_chat_path in chat_path_list:
            ...
            fs = hadoop.fs.FileSystem
            conf = hadoop.conf.Configuration() 
            path_str = 'hdfs://localhost:8020/stats/chat_stats_ver2_'+str(index)+'.csv'
            ...
            result = spark_session.sql('''
                ...
                count(timestamp) AS chats, count(distinct DATE(timestamp)) AS videoDays, 
                count(distinct channelId) AS uniqueChatters, SUM(bodylength) AS totalBodyLength
                from chat 
                group by authorChannelId''')#.cache()
            
            # UNIQUE MEMBER QUERY PLACEHOLDER
            # result = spark_session.sql('''
    ...
    if False:
        print('Read superchat file into dataframes'.ljust(100,'-'))
        index = 0
        time_list = []
        for each_chat_path in superchat_path_list:
            print(('Starting spark session #'+str(index)).ljust(100,'-'))
            spark_session = SparkSession.builder.master('local[*]').appName('session #' + str(index)).getOrCreate()
            spark_context = spark_session._sc
            ...
            fs = hadoop.fs.FileSystem
            conf = hadoop.conf.Configuration() 
            path_str = 'hdfs://localhost:8020/stats/superchat_stats_'+str(index)+'.csv'
            path = hadoop.fs.Path(path_str)
            if fs.get(conf).exists(path):
                print('exists!')
                import subprocess
            ...
            each_chat = spark_session.read.parquet(each_chat_path)
            each_chat.show()
            each_chat.createOrReplaceTempView("superchat")
            # result = spark_session.sql('''
            #     select distinct authorChannelId as channelId, first(SUBSTRING(to_date(timestamp), 1,7)) AS period,
            ...
            result = spark_session.sql('''
                select authorChannelId, count(timestamp) as superchats, AVG(significance) 
                from superchat 
                group by authorChannelId''')
            each_chat.unpersist(blocking=True)
            # result = result.na.fill(value=0,subset=["uniqueMembers"])
